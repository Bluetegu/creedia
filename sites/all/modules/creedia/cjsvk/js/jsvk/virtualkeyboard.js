/**
 * $Id: virtualkeyboard.js 539 2009-02-14 06:23:29Z wingedfox $
 * $HeadURL: https://svn.debugger.ru/repos/jslibs/Virtual%20Keyboard/tags/VirtualKeyboard.v3.6.0/virtualkeyboard.js $
 *
 * Virtual Keyboard.
 ...
 * @version $Rev: 539 $
 * @lastchange $Author: wingedfox $ $Date: 2009-02-14 09:23:29 +0300 (Сбт, 14 Фев 2009) $
 */
/**
...
var VirtualKeyboard = new function () {
  var self = this;
  self.$VERSION$ = " $HeadURL: https://svn.debugger.ru/repos/jslibs/Virtual%20Keyboard/tags/VirtualKeyboard.v3.6.0/virtualkeyboard.js $ ".match(/\/[^\.]*[\.\/]([^\/]+)\/[\w\.\s$]+$/)[1]+"."+(" $Rev: 539 $ ".replace(/\D/g,""));
  /**
  ...
  var options = {
    'layout' : null,
    'hidelayoutselect' : false,  // show/hide layout selection
    'hidekeymapselect' : false,  // show/hide keymap selection
    'nocopyright'      : false,  // show/remove copyright
    'cookies'          : true,   // set layout and keymap cookie
    'rtl'              : true    // set rtl
  }
  /**
   *  ID prefix
  ...
  **********************************************************/
  ...
  var __toggleInputDir = function (reset) {
      if (nodes.attachedInput && options.rtl) {
          var mode = reset?""
                          :(lang.rtl?'rtl'
                                    :'ltr');
  ...
  ;(function() {
      ...
      */
      ...
      options.hidelayoutselect = opts.vk_hidelayoutselect || false;
      options.hidekeymapselect = opts.vk_hidekeymapselect || false;
      options.cookies = opts.vk_cookies == undefined ? options.cookies : opts.vk_cookies;
      options.nocopyright = opts.vk_nocopyright || false;
      options.rtl = opts.vk_rtl == undefined ? options.rtl : opts.vk_rtl;
      // temporarly hard set options...
      options.cookies = false;
      options.nocopyright = true;
      options.hidelayoutselect = false;
      options.hidekeymapselect = false;
      options.rtl = false;

      if (options.cookies) {
        options.layout = DocumentCookie.get('vk_layout');
      }
      options.layout = options.layout || opts.vk_layout || null;
      ...
      */
      ...
      var render = "<div id=\"kbDesk\"><!-- --></div>";
      render += options.hidelayoutselect ? "<select id=\"kb_langselector\" style=\"display:none;\"></select>" : 
        "<select id=\"kb_langselector\"></select>";
      render += options.hidekeymapselect ? "<select id=\"kb_mappingselector\" style=\"display:none;\"></select>" : 
        "<select id=\"kb_mappingselector\"></select>"
      if (!options.nocopyright) {
	render += '<div id="copyrights" nofocus="true"><a href="http://debugger.ru/projects/virtualkeyboard" target="_blank" title="&copy;2006-2009 Debugger.ru">VirtualKeyboard '+self.$VERSION$+'</a></div>';
      }
      nodes.keyboard.innerHTML = render;
